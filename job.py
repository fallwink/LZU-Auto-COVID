...
def job_function():
    failure = False
    if os.system("python LZU-Auto-COVID-Health-Report.py >> information.txt && type information.txt >> logs.txt && type information.txt") == 0:
        if os.system("python Notify-Result.py success") != 0:
            failure = True
    else:
        failure = True
        os.system(
            "python Notify-Result.py failure && type information.txt >> logs.txt && type information.txt")
